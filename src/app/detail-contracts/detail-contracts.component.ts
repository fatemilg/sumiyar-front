...
export class DetailContractsComponent implements OnInit {
  ...
  load_detail_contract(id_contract) {
      ...
      .subscribe(
        (data: XResult) => {
          if (data.IsOK) {
            ...
            this.load_tasks_by_industry(data.Value[0].IDDefine)
          }
          else {
  ...
  load_tasks_by_industry(id_define) {
    this.visible_progress = true;
  ...
  ngOnInit() {
    this.load_detail_contract(this.id_contract);
  }
